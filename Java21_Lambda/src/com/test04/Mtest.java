...
public class Mtest {
	...
	public static void pipeline() {
		...
		class504.stream().filter(name ->name.contains("승") ).sorted()
	...
	public static void createStream() {
		//builder : 스트림에 직접적으로 원하는 값 넣을수 있다.
		//build()메소드 호출하여 스트림을 리턴
		
		Stream<String> stream01 = Stream.<String>builder().add("홍길동").add("이순신").add("유관순").build();
		stream01.forEach(System.out::println); //add하나씩 가져온다
